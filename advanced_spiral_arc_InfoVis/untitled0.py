...
#read graph data
df_authors_graph = pd.read_csv('authors_graph.txt', delimiter=',')  # Change delimiter if needed
df_authors_keys = pd.read_csv('authors_keys.txt', delimiter= '|') 
df_authors_keys = df_authors_keys.rename(columns={'923': 'node'})
df_authors_keys['node'] = df_authors_keys['node'].astype(str)

# Save to CSV
df_authors_graph.to_csv('authors_graph.csv', index=False)

#G =  nx.read_edgelist("musae_git_edges.csv", delimiter=',')
G =  nx.read_edgelist("authors_graph.csv", delimiter=',')
...
new_data_based_on_community_size = pd.merge(new_data_based_on_community_size, df_authors_keys, on='node', how='inner')
